...
/* global R, chai, describe, it, mocha, window */
'use strict';
	...
	describe('ramda library', function () {
		describe('simple operations', function () {
			it('should test this next', function () {
				...
				expect([keys.length,keys[30]]).to.be.deep.equal([]);
			});
		...
		describe('functional operations', function () {
			...
			describe('Function> R.binary :: (* → c) → (a, b → c)', function () {
				it('should return a binary argument function, ignores args 3+', function () {
					...
					expect(planar(1, 2, 3)).to.be.deep.equal([1, 2, 0]);
				});
			});
...
*/
		...
		describe('object operations', function () {
			...
			describe('Object> R.clone :: {*} → {*}', function () {
				it('should deep copy an object properties (and prototype) but reference copy functions and the prototype methods', function () {
					...
					expect(Object.keys(objNew)).to.be.deep.equal(['name', 'ref', 'method', 'girth', 'protoMethod']);
					expect(objNew.ref).to.not.be.equal(obj.ref);
					expect(objNew.ref).to.be.deep.equal(obj.ref);
					...
					expect(objNew.protoMethod()).to.be.deep.equal(['huge']);
				});
		...
		describe('list operations', function () {
			...
			describe('List> R.chain :: (a → [b]) → [a] → [b] aka flatMap', function () {
				it('should duplicate array entries into a new array', function () {
					...
					expect(R.chain(duplicate, [1, 2, 3])).to.be.deep.equal([1, 1, 2, 2, 3, 3]);
				});
	...
	describe('xray :: a → [String]', function () {
	    it('should answer with _inherits property if present', function () {
			expect(F.xray({_inherits: ['Mouse', 'Mammal']})).to.be.deep.equal(['Mouse', 'Mammal']);
		});
		it('should identify undefined', function () {
			expect(F.xray(void 0)).to.be.deep.equal([]);
		});
		it('should identify the null object', function () {
			expect(F.xray(null)).to.be.deep.equal(['Object']);
		});
		it('should identify NaN', function () {
			expect(F.xray(NaN)).to.be.deep.equal(['number']);
		});
		it('should identify Infinity', function () {
			expect(F.xray(Infinity)).to.be.deep.equal(['number']);
		});
		it('should identify -Infinity', function () {
			expect(F.xray(-Infinity)).to.be.deep.equal(['number']);
		});
		it('should identify boolean false', function () {
			expect(F.xray(false)).to.be.deep.equal(['boolean']);
		});
		it('should identify boolean true', function () {
			expect(F.xray(true)).to.be.deep.equal(['boolean']);
		});
		it('should identify a zero number', function () {
			expect(F.xray(0)).to.be.deep.equal(['number']);
		});
		it('should identify a number', function () {
			expect(F.xray(12)).to.be.deep.equal(['number']);
		});
		it('should identify a string', function () {
			expect(F.xray('a')).to.be.deep.equal(['string']);
		});
		it('should identify a regex', function () {
			expect(F.xray(/this/)).to.be.deep.equal(['RegExp']);
		});
		it('should identify the Boolean object', function () {
			expect(F.xray(Boolean(true))).to.be.deep.equal(['boolean']);
		});
		it('should identify the Number object', function () {
			expect(F.xray(Number(2))).to.be.deep.equal(['number']);
		});
		it('should identify the String object', function () {
			expect(F.xray(String('3'))).to.be.deep.equal(['string']);
		});
		it('should identify a RegExp object', function () {
			expect(F.xray(new RegExp('this', 'g'))).to.be.deep.equal(['RegExp']);
		});
		it('should identify an anonymous function', function () {
			expect(F.xray(function () {})).to.be.deep.equal(['Function']);
		});
		it('should identify a named function', function () {
			expect(F.xray(function namedFn () {})).to.be.deep.equal(['Function namedFn']);
		});
		// document, events, etc and then inherited objects
		it('should identify the Window object', function () {
			expect(F.xray(window)).to.be.deep.equal(['Window']);
		});
	});
...
function defineCode() {
	...
	F.xray = function (obj, ancestry) {
		/* jshint maxcomplexity: 7, maxdepth: 3 */
		ancestry = ancestry || { _inherits: [], found: {} };

		var typeOf = typeof obj,
			outIf = function (check, type) {
				if (check) {
					if (type) {
						ancestry._inherits.push(type);
					}
					throw new Error(type);
				}
			},
			inheritIfMatch = function (type, regex, prefix) {
				prefix = prefix || '';
				var match = type.match(regex);
				if (match) {
					type = prefix + match[1];
					inherit(type);
					// TODO follow prototype chain up
					throw new Error(type);
				}
			},
			inherit = function (type) {
				if (!(type in ancestry.found)) {
					ancestry.found[type] = true;
					ancestry._inherits.push(type);
				}
			};
		...
		try {
			outIf(obj === undefined);
			outIf(obj === null, 'Object');
			outIf(!/^(object|function)$/.test(typeOf), typeOf);
			if (typeOf === 'function') {
				inheritIfMatch(obj.toString(), /^function ([^\(]+)\(/, 'Function ');
				// will need to see if function itself is a constructor later...
				outIf(true, 'Function');
			}
			if ('_inherits' in obj) {
			...
			if (obj.constructor && obj.constructor.toString) {
				inheritIfMatch(obj.constructor.toString(), /^function ([^\(]+)\(/);
			}
			if ('toString' in obj) {
				inheritIfMatch(obj.toString(), /\[object (.+)\]/);
			}
		}
		finally {
			return ancestry._inherits;
		}
	};
